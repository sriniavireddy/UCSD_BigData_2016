...
#%%time
# Your code here
f = open("../Data/hw2-files-20gb.txt","r")
...
def safe_parse(raw_json):
    try:
        ...
        return (tweet['user']['id_str'],tweet['text'])
    except:
        return "error"

users = RDD.map(lambda x: safe_parse(x))            .filter(lambda x: x!="error").cache()
#users.take(5)
...
print users.map(lambda x:x[0]).distinct().count()
...
# your code here
users_group = users.map(lambda x: (x[0],find_partition_id(x[0]))).cache()
#user_group_map = sc.broadcast(users_group.collectAsMap())
group_RDD = users_group.map(lambda x: (x[1],1))                 .reduceByKey(lambda x,y: x+y)                 .sortByKey(True)
...
sorted_filtered_tokens_count = filtered_tokens.map(lambda x: (len(x[1]),x[0]))                                    .sortByKey(False)                                    .map(lambda (b,a):(a.encode('UTF-8'),b))
...
def token_group(token):
    group_list = map(lambda x: user_group_map.value[x], token[1])
    return zip([token[0]] * len(token[1]), group_list, [len(token[1])] * len(token[1]))
    

group_tokens = filtered_tokens.flatMap(lambda x: token_group(x)).cache()
#print group_tokens.collect()
for i in xrange(0,8):
    reduce_rdd = group_tokens.filter(lambda x: x[1] == i).map(lambda x:((x[0],x[2]),1)).reduceByKey(lambda x,y:x+y)\
                         .map(lambda x:((-get_rel_popularity(x[1],x[0][1]),x[0][0]),x[0][0]))\
                         .sortByKey(True)\
                         .map(lambda x: (x[1].encode('UTF-8'),-x[0][0]))
    #print reduce_rdd.first()
    print_tokens(reduce_rdd.take(10),i)
#get_ipython().run_cell_magic(u'time', u'', u"user_group_map = sc.broadcast(users_group.collectAsMap())\ndef token_group(token):\n    group_list = map(lambda x: user_group_map.value[x], token[1])\n    return zip([token[0]] * len(token[1]), group_list, [len(token[1])] * len(token[1]))\n    \n\ngroup_tokens = filtered_tokens.flatMap(lambda x: token_group(x)).cache()\n#print group_tokens.collect()\nfor i in xrange(0,8):\n    reduce_rdd = group_tokens.filter(lambda x: x[1] == i).map(lambda x:((x[0],x[2]),1)).reduceByKey(lambda x,y:x+y)\\\n                         .map(lambda x:((-get_rel_popularity(x[1],x[0][1]),x[0][0]),x[0][0]))\\\n                         .sortByKey(True)\\\n                         .map(lambda x: (x[1].encode('UTF-8'),-x[0][0]))\n    #print reduce_rdd.first()\n    print_tokens(reduce_rdd.take(10),i)")
